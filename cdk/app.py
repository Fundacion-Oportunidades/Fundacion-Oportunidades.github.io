...
Route53Stack(app, 'oportunidadesRoute53Stack',
    ...
    domain_name= os.getenv('oportunidades_domain_name') ,
    env=cdk.Environment(account=os.getenv('oportunidades_aws_account'), 
                        region=os.getenv('oportunidades_main_region'))
)
...
CloudFrontS3Stack(app, 'oportunidadesCloudFrontS3Stack',
    domain_name=os.getenv('oportunidades_domain_name'),
    certificate_arn=os.getenv('oportunidades_certificate_arn'),
    env=cdk.Environment(account=os.getenv('oportunidades_aws_account'), 
                        region=os.getenv('oportunidades_main_region'))
)
...
AliasRecordsStack(app, 'oportunidadesCloudFrontS3Stack',
    ...
    domain_name=os.getenv('oportunidades_domain_name'),
    env=cdk.Environment(account=os.getenv('oportunidades_aws_account'), region=os.getenv('oportunidades_main_region'))
)
